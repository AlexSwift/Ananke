...
public:
	...
	virtual const char  *GetName( void ) const;	// get channel name
	virtual const char  *GetAddress( void ) const; // get channel IP address as string
	virtual float		GetTime( void ) const;	// current net time
	virtual float		GetTimeConnected( void ) const;	// get connection time in seconds
	virtual int			GetBufferSize( void ) const;	// netchannel packet history size
	virtual int			GetDataRate( void ) const; // send data rate in byte/sec
	
	virtual bool		IsLoopback( void ) const;	// true if loopback channel
	virtual bool		IsTimingOut( void ) const = 0;	// true if timing out
	virtual bool		IsPlayback( void ) const;	// true if demo playback
